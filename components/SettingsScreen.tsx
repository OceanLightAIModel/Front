import { getUserProfile, updateUserPrefs, deleteUserAccount, logout } from './api';
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        // 테마/다크모드도 다양한 키를 지원
        setChatTheme(userData.chat_theme ?? userData.chatTheme ?? false);
        setDarkMode(userData.dark_mode ?? userData.darkMode ?? false);
      } catch (e) {
  ...
  return (
    <SafeAreaView style={[styles.container, darkMode && styles.containerDark]}>
      ...
      {/* ✅ 삭제 성공 알림 모달 (확인 버튼 누르면 로그아웃 후 로그인 화면) */}
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, darkMode && styles.modalCardDark]}>
            <MaterialIcons
              ...
              style={{ alignSelf: 'center', marginBottom: 6 }}
            />
            <Text style={[styles.modalTitle, darkMode && styles.modalTitleDark]}>{successModal.title}</Text>
            <Text style={[styles.modalMessage, darkMode && styles.modalMessageDark]}>{successModal.message}</Text>
            <View style={styles.modalButtons}>
              <TouchableOpacity
                style={[styles.modalBtn, styles.modalPrimaryBtn]}
                onPress={async () => {
                  try {
                    // 백엔드 로그아웃(토큰 서버측 폐기) + 로컬 토큰 정리
                    await logout().catch(() => {});
...
const styles = StyleSheet.create({
  ...
  // ✅ 성공 모달용 기본(primary) 버튼
  modalPrimaryBtn: { backgroundColor: '#0080ff' },
